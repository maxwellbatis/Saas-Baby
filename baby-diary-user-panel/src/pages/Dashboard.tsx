...
import { Baby, Image, User, Calendar, Trophy, TrendingUp, Clock, Heart, Utensils, Bath, Puzzle, Pill, Activity as ActivityIcon, Bot } from "lucide-react";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        // Buscar memórias
        const memoriesRes = await fetch(`http://localhost:3000/api/user/memories?babyId=${baby.id}`, {
        ...
        // Buscar marcos
        const milestonesRes = await fetch(`http://localhost:3000/api/user/milestones?babyId=${baby.id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        ...
        // Buscar atividades
        const activitiesRes = await fetch(`http://localhost:3000/api/user/babies/${baby.id}/activities`, {
          headers: { Authorization: `Bearer ${token}` },
        });
